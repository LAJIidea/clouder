...
import { Command, CommandContribution, CommandRegistry, MessageService, nls, Progress, QuickInputService, QuickPickItem, MenuContribution, MenuModelRegistry } from "@theia/core";
...
import { SocketIoTransportProvider } from "./socket-io-transpose";
...
@injectable()
export class CollaborationFrontendContribution implements CommandContribution, MenuContribution {
  ...
  protected async onStatusDefaultClick(): Promise<void> {
    ...
    items.push({
      label: nls.localize('theia/collaboration/joinRoom', 'Join Collabration Session'),
  ...
  protected async onStatusSharedClick(code: string): Promise<void> {
    ...
    items.push({
      ...
      detail: nls.localize('theia/collaboration/endDetail', 'Terminate the session, case content sharing, and revoke access for others.'),
      iconClasses: codiconArray('circle-slash'),
      execute: () => this.currentInstance?.dispose()
    });
    await this.quickInputService?.showQuickPick(items, {
      placeholder: nls.localize('theia/collaboration.wahtToDo', 'What would you like to do with other collaborators?')
    });
  }
  ...
  protected async displayCopyNotification(code: string, firstTime = false): Promise<void> {
    ...
    const notification = nls.localize('theia/collaboration/copiedInvitation', 'Invitation code copied to clipbord');
    if (firstTime) {
  ...
  registerMenus(menus: MenuModelRegistry): void {
    menus.registerMenuAction(CommonMenus.VIEW, {
      commandId: CollaborationCommands.CREATE_ROOM.id,
      label: CollaborationCommands.CREATE_ROOM.label
    });
    menus.registerMenuAction(CommonMenus.VIEW, {
      commandId: CollaborationCommands.JOIN_ROOM.id,
      label: CollaborationCommands.JOIN_ROOM.label
    });
  }
}